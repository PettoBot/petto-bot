...
src/
  ...
  utils/
    ...
    levelProgressBar.js              buildProgressBar() — a 9-custom-emoji 10-segment bar if emojis.js has them, else
                                    falls back to a plain block-character bar (see "Leveling" below for which 9 to make)
...
**The progress bar** (`utils/levelProgressBar.js`) is built for a 9-custom-emoji, 10-segment bar exactly like bli's (3 positions — start/middle/end — × 3 fill states — full/half/empty) and automatically uses it once those are added to `utils/emojis.js`; until then it falls back to a plain `█`/`░` block bar, which is what's live right now. **To upgrade it, make 9 emojis and hand me the IDs**: `BAR_START_FULL`, `BAR_START_HALF`, `BAR_START_EMPTY` (the bar's left cap), `BAR_MID_FULL`, `BAR_MID_HALF`, `BAR_MID_EMPTY` (the repeating middle segment), `BAR_END_FULL`, `BAR_END_HALF`, `BAR_END_EMPTY` (the right cap) — same shape as a typical rounded-pill progress bar split into 3 pieces, just three fill states of each piece.
