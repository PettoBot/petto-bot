...
async function handleSetupModal(interaction) {
  ...
  const logChannel = selectedChannel(
    fields,
    'setup_log_channel'
  );

  const welcomeChannel = selectedChannel(
    fields,
    'setup_welcome_channel'
  );

  const mode = fields.getRadioGroup(
    'setup_moderation_mode',
    true
  );

  const features = [
    ...fields.getStringSelectValues('setup_features'),
  ];

  const prefix = fields
    .getTextInputValue('setup_prefix')
    .trim();

  if (!prefix || prefix.length > 5 || /\s/.test(prefix)) {
    await interaction.reply({
      content:
        'The prefix must be 1 to 5 characters and cannot contain spaces.',
  ...
  try {
    ...
    // Update the in-memory prefix cache immediately.
    // This prevents the old prefix from continuing to work
    ...
    await upsertAutomodConfig(guild.id, {
      anti_spam_enabled:
        !disabled &&
        features.includes('anti-spam'),

      anti_raid_enabled:
        !disabled &&
        features.includes('anti-raid'),
    ...
    const existingWelcome = await getMemberConfig(
      guild.id
    ).catch(() => null);

    const welcomeEnabled =
      features.includes('welcome') &&
      Boolean(welcomeChannel);
    ...
    if (
      welcomeEnabled &&
      !existingWelcome?.welcome_message &&
      !existingWelcome?.welcome_embed_template
    ) {
      welcomePatch.welcome_message =
        'Welcome {user.mention} to {server.name}!';
    }

    await upsertMemberConfig(
      guild.id,
      welcomePatch
    );

    let logLine =
      'Audit logs were left unchanged.';

    if (
      features.includes('logs') &&
      logChannel
    ) {
      const added = await configureLogChannel(
        interaction.client,
        guild,
        logChannel
      );
      ...
      logLine =
        `Audit logs are routed to <#${logChannel.id}> ` +
        `(${added || 'already configured'} event categories).`;

    } else if (
      features.includes('logs')
    ) {
      logLine =
        'Audit logs were not enabled because no log channel was selected.';
    }
    ...
    if (disabled) {
      officialAutoMod = {
        reason: 'disabled_by_setup',
      };

    } else if (
      !features.includes('official-automod')
    ) {
      officialAutoMod = {
        reason: 'not_selected',
      };
      ...
      officialAutoMod =
        await syncGuildAutoMod(guild).catch(
          (err) => {
            logger.warn(
              `[AutoMod] Setup synchronization failed for guild ${guild.id}: ${err.message}`
            );
    ...
    const setupChannel =
      await ensureAdminSetupChannel(
        guild
      ).catch((err) => {
        logger.warn(
          `Could not create private setup channel in guild ${guild.id}: ${err.message}`
        );
    ...
    const lines = [
      ...
      welcomeEnabled
        ? `Welcome messages are enabled in <#${welcomeChannel.id}>.`
    ...
    logger.error(
      `Petto setup failed in guild ${guild.id}:`,
      err
    );
    ...
    await interaction.editReply({
      components: [
        textCard(
          `${EMOJI.DENY} Setup could not be completed: ${err.message}`,
          0xfe6465
        ),
      ],
      flags: MessageFlags.IsComponentsV2,
